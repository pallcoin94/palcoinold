...
#if defined(NDEBUG)
# error "Palcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x6e5d27d84ba297dbbc08a01ad7632ced897896b018f9c1e29c71d0f8f2fdd01b");
...
const string strMessageMagic = "Palcoin Signed Message:\n";
...
{
    // Palcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Palcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Palcoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Palcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Palcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Palcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x4246b69b2dcac8bda6990175074cac539b06603cd078a4b154c81b089f2d09e5");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "this is just a test so no news :P";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0461642f3ff25604551a8e0f805b39604c37476b7a2458432a8b14e46b36c84deba0374b53d64897180f6419b982163774b85d69b759d494b8e122084645b645b2") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1546985620;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084774016;

        if (fTestNet)
        {
            block.nTime    = 1546985583;
            block.nNonce   = 386689863;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x730ab384ad471dcc8f7ab0be01bfdb569cbaf4a734e85958a1a3ca5584d19a22"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xab, 0xc3, 0xba, 0xd3 }; // Palcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PalcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PalcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PalcoinMiner(CWallet *pwallet)
{
    printf("PalcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("palcoin-miner");
    ...
    try { loop {
        ...
        printf("Running PalcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PalcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PalcoinMiner, pwallet));
}
